...
#data = pd.read_csv(r"D:\work\water_potability.csv")

#test_size = yaml.safe_load(open('params.yaml'))['data_collection']['test_size']

# train_data,test_data = train_test_split(data, test_size= test_size, random_state=22)

def split_data(data : pd.DataFrame, test_size : float)-> tuple[pd.DataFrame, pd.DataFrame]:
    try:
        return train_test_split(data, test_size= test_size, random_state=22)
    except Exception as e:
